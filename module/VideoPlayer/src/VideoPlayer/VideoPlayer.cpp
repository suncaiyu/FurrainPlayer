...
fprintf(stderr, "%s mIsQuit=%d mIsPause=%d \n", __FUNCTION__, mIsQuit, mIsPause);
    while (1)
    {
        ...
        if (seek_req)
        {
            ...
            if (videoStream >= 0) {
                AVStream *as = pFormatCtx->streams[videoStream];
                double fps = as->avg_frame_rate.den == 0 ? 0
                                                         : (double) as->avg_frame_rate.num
                                                               / (double) as->avg_frame_rate.den;
                if (fps == 0) {
            ...
            if (av_seek_frame(pFormatCtx, stream_index, seek_target, AVSEEK_FLAG_FRAME |AVSEEK_FLAG_BACKWARD) < 0)
            {
